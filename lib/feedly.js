...
var tabs = require('sdk/tabs'),
    ...
    { defer, race } = require("sdk/core/promise"),
            ...
            creds.forEach((cred) => {
                ...
                let req = Request({
                    ...
                    onComplete: function(response) {
                        if(!resp.json || !response.json.access_token) {
                            d.reject();
                        }
                        else {
                ...
                onComplete: function(resp) {
                    if(!resp.json || !resp.joson.items) {
                        ...
                        let checkURL = (article) => {
                            let p = defer();
                            resolveURLIfNeeded(removeTrackingGarbage(article.alternate[0].href)).then((resolvedURL) => {
                                console.log(resolvedURL+" == "+url);
                                if(resolvedURL == url)
                                    p.resolve(article.id);
                            });
                            return p.promise;
                        }
                        // Does this leak if none of the promises ever resolves? Let's just trust the browser not to.
                        race(resp.json.items.map(checkURL)).then((id) => d.resolve(id));
                    }
                }
            });
            req.get();
